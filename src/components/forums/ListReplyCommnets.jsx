...
const ListReplyCommnets = ({ replyComments, commentFatherID }) => {
  return replyComments !== []
    ? replyComments.map((replyC) =>
        /* eslint no-underscore-dangle:0 */
        replyC.commentId === commentFatherID ? (
          <div className='media mt-3' key={replyC._id}>
            <div className='mr-3'>
              <img
            ...
            <div className='media-body'>
              <h6 className='mt-0'>{replyC.user?.name}</h6>
              <p>{replyC.comment}</p>
            </div>
          </div>
        ) : (
...
export default ListReplyCommnets;
