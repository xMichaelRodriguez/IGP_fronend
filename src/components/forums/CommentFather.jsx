import React, { useEffect, useState } from 'react';
import { FaTrash } from 'react-icons/fa';
import { useDispatch, useSelector } from 'react-redux';
import { useParams } from 'react-router';
import { startForumDelete, uiRemoveError } from '../../actions/authActios';
import CommentBox from './CommentBox.jsx';
import ListReplyCommnets from './ListReplyCommnets.jsx';

const CommentFather = ({ commentFather, replyComments }) => {
  const dispatch = useDispatch();
  const { foroId } = useParams();
  const { uid } = useSelector((state) => state.auth);
  const { user } = useSelector((state) => state.userForum);
  const [commentActive, setCommentActive] = useState('');
  const [commentBoxState, setCommentBoxState] = useState(false);
  const handleDeleteComment = (id) => {
    dispatchEvent(startForumDelete(id, foroId));
  };
  const handleReplyComment = (id) => {
    if (Object.entries(user).length === 0) return;

    setCommentActive(id);
    setCommentBoxState(true);
  };
  const cancelComment = (id) => {
    if (String(commentFather._id) === String(id)) {
      setCommentBoxState(false);
      setCommentActive('');
    }
  };

  useEffect(() => {
    if (!commentBoxState) {
      dispatch(uiRemoveError(''));
    }
    return () => {
      uiRemoveError();
    };
  }, [dispatch, commentBoxState]);
  return (
    <li className='media  '>
      <img
        src='data:image/png;base64,'
        className='mr-3 '
        style={{ height: '30px', width: '30px' }}
        alt='avatar'
      />
      <div className='media-body'>
        <h5 className='mt-0 mb-1'>{commentFather.user?.name}</h5>
        <p className='mb-0 '>{commentFather.comment}</p>
        <div className='m-2'>
          <small className=''>
            <button
              className='btn btn-link'
              type='button'
              onClick={() => handleReplyComment(commentFather._id)}
            >
              Responder
            </button>
          </small>
          <small className=''>
            {uid ? (
              <button
                className='btn btn-link btn-sm text-danger'
                /* eslint no-underscore-dangle:0 */
                onClick={() => handleDeleteComment(commentFather._id)}
                type='button'
              >
                <FaTrash /> Eliminar
              </button>
            ) : (
              ''
            )}
          </small>
        </div>
        {commentBoxState ? (
          <>
            <CommentBox foroId={commentActive} context={'reply'} />
            <button
              type='button'
              className='btn btn-outline-primary'
              onClick={() => cancelComment(commentActive)}
            >
              cancelar
            </button>
          </>
        ) : (
          ''
        )}

        <ListReplyCommnets
          replyComments={replyComments}
          commentFatherID={commentFather._id}
        />
      </div>
    </li>
  );
};

export default CommentFather;
